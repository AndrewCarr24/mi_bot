"""Build / extend the dsRAG KnowledgeBase from SEC filings under data/parsed/.

Each filing is ingested as its own dsRAG document — the filename stem
(`TICKER_FORM_PERIOD`) becomes the `doc_id`, which the agent later passes
as a metadata filter to scope retrieval to one filing.
...
Stack:
    ...
    Reranker:        NoReranker — RSE alone handles relevance scoring
                     for our use case; no external rerank API needed.
    AutoContext LLM: OpenAIChatAPI pointed at DeepSeek's OpenAI-compatible
                     endpoint with model="deepseek-chat" (non-reasoning
                     backend; v4-flash's default thinking mode rejects
                     instructor's forced tool_choice and falls back to
                     generic "Window N-M" section titles).
    VectorDB:        ChromaDB.
...
# Make our custom subclass registerable before constructing the KB.
